...
bool Rectangle2D::intersect(Point2D<int>& pt0, Point2D<int>& pt1) const {
    ...
    if(is_point_inside(pt0)) {
        if(is_point_inside(pt1)) return true;       // pt0 & pt1: both in
        else return perfom_intersection(pt0, pt1);  // pt0: in; pt1: out
    }
    else {
        if(is_point_inside(pt1)) return perfom_intersection(pt1, pt0);  // pt0: out; pt1: in
...
// Pre_condition: pt_in is inside the rectangle (including the borders),
// pt_out is outside the rectangle
bool Rectangle2D::perfom_intersection(const Point2D<int>& pt_in, Point2D<int>& pt_out) const {
    ...
    case intersection_zone::top_left:
        // intersect with the left and top segments and update pt_out with the
        // intersecting point
    ...
    case intersection_zone::top_right:
        // intersect with the right and top segments and update pt_out with the
        // intersecting point
        if(intersect_with_segment(pt_in, pt_out, segment_type::right)) return true;
        if(intersect_with_segment(pt_in, pt_out, segment_type::top))   return true;
        break;
    case intersection_zone::top:
        // intersect with the top segment and update pt_out with the
        // intersecting point
        if(intersect_with_segment(pt_in, pt_out, segment_type::top)) return true;
        break;
    case intersection_zone::bottom_left:
        // intersect with the left and bottom segments and update pt_out with the
        // intersecting point
        if(intersect_with_segment(pt_in, pt_out, segment_type::left))   return true;
        if(intersect_with_segment(pt_in, pt_out, segment_type::bottom)) return true;
        break;
    case intersection_zone::bottom_right:
        // intersect with the right and bottom segments and update pt_out with the
        // intersecting point
        if(intersect_with_segment(pt_in, pt_out, segment_type::right))  return true;
        if(intersect_with_segment(pt_in, pt_out, segment_type::bottom)) return true;
        break;
    case intersection_zone::bottom:
        // intersect with the bottom segment and update pt_out with the
        // intersecting point
        if(intersect_with_segment(pt_in, pt_out, segment_type::bottom)) return true;
        break;
    case intersection_zone::left:
        // intersect with the left segment and update pt_out with the
        // intersecting point
        if(intersect_with_segment(pt_in, pt_out, segment_type::left)) return true;
        break;
    case intersection_zone::right:
        // intersect with the right segment and update pt_out with the
        // intersecting point
        if(intersect_with_segment(pt_in, pt_out, segment_type::right)) return true;
        break;
    deafult:
