...
public:
    ...
    void UpdateDynamicEllipse(const std::unique_ptr<Ellipse2D>& ellipse);
    void UpdateBaseEllipse(const std::unique_ptr<Ellipse2D>& elp);

private:

    osg::ref_ptr<osg::Vec2dArray>               m_sweep_vertices;       // for displaying the seep ellipse
    std::vector<osg::ref_ptr<osg::DrawArrays>>  m_sweep_arrays;         // draw arrays for m_sweep_vertices
