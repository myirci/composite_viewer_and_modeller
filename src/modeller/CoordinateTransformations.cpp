...
CoordinateTransformations::CoordinateTransformations(double fy, int w, int h, double n, double f) : fovy(fy), width(w), height(h), near(n), far(f) {
...
void CoordinateTransformations::convert_from_viewport_coordinates_to_normalized_device_coordinates(const osg::Vec2d& vp_coord, osg::Vec2d& ndc_coord) {

    ndc_coord.x() = (2.0 / static_cast<double>(width))*vp_coord.x();
    ndc_coord.y() = (2.0 / static_cast<double>(height))*vp_coord.y();
}

void CoordinateTransformations::convert_from_projected_coordinates_to_normalized_device_coordinates(const osg::Vec2d& prj_coord, osg::Vec2d& ndc_coord) {

    ndc_coord.x() = prj_coord.x() / (aspect * c1);
    ndc_coord.y() = prj_coord.y() / c1;
}

void CoordinateTransformations::convert_from_normalized_device_coordinates_to_projected_coordinates(const osg::Vec2d& ndc_coord, osg::Vec2d& prj_coord) {
...
void CoordinateTransformations::construct_perpective_projection_matrix(osg::Matrixd& proj_mat) {
...
void CoordinateTransformations::construct_viewport_mapping_matrix(osg::Matrixd& vp_mat) {
