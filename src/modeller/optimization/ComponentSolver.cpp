...
ComponentSolver::ComponentSolver(const std::shared_ptr<ProjectionParameters>& pp) : m_lfpts(nullptr), m_pp(pp) {
...
std::pair<double, double> ComponentSolver::quadratic_root_finder_x(double x, double a1, double a2) const {
    ...
    double g = m_pp->far + m_pp->near;
    double a = 2*x;
    double b = a*(a1+a2+2*g) - m_pp->far*(m_lfpts->at(1).x() + m_lfpts->at(2).x());
    double c = a*(a1*a2+g*(a1+a2+g)) - m_pp->far*(m_lfpts->at(1).x()*(a2+g) + m_lfpts->at(2).x()*(a1+g));
...
std::pair<double, double> ComponentSolver::quadratic_root_finder_y(double y, double a1, double a2) const {
    ...
    double g = m_pp->far + m_pp->near;
    double a = 2*y;
    double b = a*(a1+a2+2*g) - m_pp->far*(m_lfpts->at(1).y() + m_lfpts->at(2).y());
    double c = a*(a1*a2+g*(a1+a2+g)) - m_pp->far*(m_lfpts->at(1).y()*(a2+g) + m_lfpts->at(2).y()*(a1+g));
    double sqrt_delta = std::sqrt(b*b - 4*a*c);
    return std::make_pair((-b + sqrt_delta) / (2*a), (-b - sqrt_delta) / (2*a));
}
...
std::pair<double, double> ComponentSolver::quadratic_root_finder_z(double z, double a1, double a2) const {
    ...
    double g = m_pp->far + m_pp->near;
    double a = 2*z;
    double b = a*(a1+a2+2*g) + 2*m_pp->far*m_pp->near;
    double c = a*(a1*a2+g*(a1+a2+g)) + m_pp->far*m_pp->near*(a1+a2+2*g);
    double sqrt_delta = std::sqrt(b*b - 4*a*c);
    return std::make_pair((-b + sqrt_delta) / (2*a), (-b - sqrt_delta) / (2*a));
}
...
void ComponentSolver::calculate_bottom_center_point(osg::Vec3d& cbt, double z1, double z2) const {
    ...
    double g = m_pp->far + m_pp->near;
    cbt.set(0.5*m_pp->far*(m_lfpts->at(1).x()/(z1+g) + m_lfpts->at(2).x()/(z2 + g)),
            0.5*m_pp->far*(m_lfpts->at(1).y()/(z1+g) + m_lfpts->at(2).y()/(z2 + g)),
           -0.5*m_pp->far*m_pp->near*(1/(z1+g) + 1/(z2+g)));
}

void ComponentSolver::calculate_top_center_point(const osg::Vec3d& cbt, osg::Vec3d& ctp, double z0, double z3) const {

    double g = m_pp->far + m_pp->near;
    ctp = cbt + osg::Vec3d(((m_pp->far*m_lfpts->at(3).x())/(z3+g)) - ((m_pp->far*m_lfpts->at(0).x())/(z0+g)),
                           ((m_pp->far*m_lfpts->at(3).y())/(z3+g)) - ((m_pp->far*m_lfpts->at(0).y())/(z0+g)),
                           ((m_pp->far*m_pp->near)/(z0+g)) - ((m_pp->far*m_pp->near)/(z3+g)));
}

void ComponentSolver::calculate_P0(osg::Vec3d& P0, double z0) const {

    P0[0] = (m_pp->far  * m_lfpts->at(0).x()) / (z0 + m_pp->far + m_pp->near);
    P0[1] = (m_pp->far  * m_lfpts->at(0).y()) / (z0 + m_pp->far + m_pp->near);
    P0[2] = (-m_pp->far * m_pp->near)        / (z0 + m_pp->far + m_pp->near);
}

void ComponentSolver::calculate_P1(osg::Vec3d& P1, double z1) const {

    P1[0] = (m_pp->far  * m_lfpts->at(1).x()) / (z1 + m_pp->far + m_pp->near);
    P1[1] = (m_pp->far  * m_lfpts->at(1).y()) / (z1 + m_pp->far + m_pp->near);
    P1[2] = (-m_pp->far * m_pp->near)        / (z1 + m_pp->far + m_pp->near);
}

void ComponentSolver::calculate_P2(osg::Vec3d& P2, double z2) const {

    P2[0] = (m_pp->far*m_lfpts->at(2).x())/(z2+m_pp->far+m_pp->near);
    P2[1] = (m_pp->far*m_lfpts->at(2).y())/(z2+m_pp->far+m_pp->near);
    P2[2] = (-m_pp->far*m_pp->near)/(z2+m_pp->far+m_pp->near);
}

void ComponentSolver::calculate_P3(osg::Vec3d& P3, double z3) const {

    P3[0] = (m_pp->far*m_lfpts->at(3).x())/(z3+m_pp->far+m_pp->near);
    P3[1] = (m_pp->far*m_lfpts->at(3).y())/(z3+m_pp->far+m_pp->near);
    P3[2] = (-m_pp->far*m_pp->near)/(z3+m_pp->far+m_pp->near);
}
