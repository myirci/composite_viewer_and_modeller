...
bool OsgWxFrame::usrLoadImageFile(const wxString& fpath) {
    ...
    if(m_bgcam.valid()) m_bgcam = NULL;
    if(m_bgeode.valid()) m_bgeode = NULL;

    // create a textured quad with the given image as texture
    wxSize img_size;
    osg::Geode* bg_image = create_textured_quad(fpath.ToStdString(), img_size);
    ...
    m_canvas->UsrInitializeModeller(m_ppp, utilityInsertAfter(fpath, wxT('.'), wxT("_binary")));
